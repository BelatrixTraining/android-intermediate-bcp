...
public class CRUDOperations {
	...
	public void addNote(NoteDbEntity noteDbEntity)
	{
		 SQLiteDatabase db = helper.getWritableDatabase(); //modo escritura
		 ContentValues values = new ContentValues();
		 values.put(MyDatabase.KEY_NAME, noteDbEntity.getName());
		 values.put(MyDatabase.KEY_DESC, noteDbEntity.getDescription());
		 values.put(MyDatabase.KEY_PATH, noteDbEntity.getPath());
		 
		 db.insert(MyDatabase.TABLE_NOTES, null, values);
		 db.close();
	}
	
	public NoteDbEntity getNote(int id)
	{
		...
		NoteDbEntity noteDbEntity = new NoteDbEntity(
				nid, name, desc,path);
		db.close();
		return noteDbEntity;
	}
	
	public List<NoteDbEntity> getAllNotes()
	{
		List<NoteDbEntity> lst =new ArrayList<NoteDbEntity>();
		...
		if(cursor.moveToFirst())
		{
			do
			{
				NoteDbEntity contact =new NoteDbEntity();
	...
	//--------------------------------------------
	public int updateNote(NoteDbEntity noteDbEntity)
	{
		...
		int row =db.update(MyDatabase.TABLE_NOTES,
				values,
				MyDatabase.KEY_ID+"=?",
				new String[]{String.valueOf(noteDbEntity.getId())});
		db.close();
	...
	//--------------------------------------------
	public int deleteNote(NoteDbEntity noteDbEntity)
	{
		 SQLiteDatabase db = helper.getWritableDatabase(); 
